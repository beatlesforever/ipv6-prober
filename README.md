...
- **丰富的子模式**: 分片 4 种模式（完整/不完整/重叠/微分片）、路由头 2 种模式（segleft=1/0）、异常顺序 4 种违规
...
```bash
# 路由扩展头探测（2 种模式：Type 0 已废弃 RFC 5095）
...
ipv6_probe_tool/
  ...
  README.md                 # 项目说明（本文件）
  探测报文设计说明.md          # 详细设计文档（IPv6 基础 + 6种报文原理 + 代码解析）
  探测功能测试报告.md          # 18项功能测试报告（含完整测试步骤和数据）
```
...
A: 使用 `--dry-run` 参数，工具会调用 Scapy 的 `show2()` 方法显示组装后的报文详细结构（包含计算后的校验和和 Next Header 字段）。
...
- `complete` / `tiny`：正常可通过, 分片成功后重组回复 (RTT 随分片数增加)
- `incomplete`：目标无法完成重组, 不回复
- `overlap`：Windows 检测到重叠冲突后直接丢弃

### Q: routing 探测 type0-segleft0 和 type0-segleft1 有什么区别？

A: segleft=1 强制目标处理路由头, Windows 返回 Parameter Problem (符合 RFC 5095); segleft=0 目标忽略路由头, 等同于 normal。两者响应差异可判断目标 RFC 5095 合规性。

### Q: 为什么我换了 IP 地址工具还能用？

A: 工具通过参数指定目标和伪造源地址, 与具体 IP 无关。只需更新 `--target` 和 `--spoofed-src` 参数即可切换实验环境。
