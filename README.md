...
- **伪造源地址预设**: 3 种安全分类（document/link-local/multicast）加自定义地址
...
```bash
# 使用预设类别（3种安全分类）
sudo python3 main.py --target 2001:db8::1 --probe-type spoofed-src --spoof-type document
sudo python3 main.py --target 2001:db8::1 --probe-type spoofed-src --spoof-type link-local
sudo python3 main.py --target 2001:db8::1 --probe-type spoofed-src --spoof-type multicast
...
| 参数                | 说明                                                                                | 默认值                | 适用探测类型         |
| ----------------- | --------------------------------------------------------------------------------- | ------------------ | -------------- |
| `--spoofed-src`   | 自定义伪造源 IPv6 地址                                                                    | `2001:db8:dead::1` | spoofed-src    |
| `--spoof-type` | 伪造源地址预设（document/link-local/multicast） | 无 | spoofed-src |
...
| `spoofed-src` | 3种预设 + 自定义 | IPv6(src=fake) / ICMPv6EchoRequest |
