...
IPv6 Abnormal Header Active Prober 是一个基于 Python 3 和 Scapy 的命令行工具，用于构造不同类型的 IPv6 探测报文，发送到指定 IPv6 目标地址，并记录目标网络或主机的响应情况。该工具支持多种探测类型，包括普通 ICMPv6 探测、伪造源地址探测、多层扩展头链探测、分片头探测、路由扩展头探测以及异常扩展头顺序探测。
...
- **可变扩展头链长度**: 通过 `--chain-len` 参数控制，支持网络测量实验
- **丰富的结果字段**: 包含 ICMPv6 type/code、RTT、Hop Limit 等详细信息
- **追加写入模式**: 支持 `--append` 参数，多次实验结果可合并
- **可追踪的探测报文**: ICMPv6 Echo Request 包含 probe\_id、seq 和 payload
...
# 指定网卡接口
sudo python3 main.py --target 2001:db8::1 --probe-type normal --iface eth0
...
| 探测类型             | 说明                          | 报文结构                                            |
| ---------------- | --------------------------- | ----------------------------------------------- |
| `normal`         | 普通 IPv6 ICMPv6 Echo Request | IPv6 / ICMPv6EchoRequest                        |
| `spoofed-src`    | 伪造源地址探测                     | IPv6(src=fake) / ICMPv6EchoRequest              |
| `ext-chain`      | 多层扩展头链探测                    | IPv6 / HopByHop / DestOpt×N / ICMPv6EchoRequest |
| `fragment`       | IPv6 分片头探测                  | IPv6 / Fragment / ICMPv6EchoRequest             |
| `routing`        | IPv6 路由扩展头探测（Type 0 已废弃）    | IPv6 / Routing(type=0) / ICMPv6EchoRequest      |
| `abnormal-order` | 异常扩展头顺序探测                   | IPv6 / DestOpt / HopByHop / ICMPv6EchoRequest   |
...
pkt = IPv6(dst="2001:db8::1") / ICMPv6EchoRequest(id=probe_id, seq=seq) / Raw(load=payload)

# 扩展头链（chain_len=3）
pkt = IPv6(dst="2001:db8::1") / IPv6ExtHdrHopByHop() / IPv6ExtHdrDestOpt() / IPv6ExtHdrDestOpt() / ICMPv6EchoRequest()

# 分片头
pkt = IPv6(dst="2001:db8::1") / IPv6ExtHdrFragment(offset=0, m=0) / ICMPv6EchoRequest()
```

每个 ICMPv6 Echo Request 包含可追踪的 payload：
...
A: 使用 `--dry-run` 参数，工具会调用 Scapy 的 `show()` 方法显示报文详细结构。

### Q: 结果文件被覆盖了怎么办？

A: 使用 `--append` 参数可以追加写入，避免覆盖之前的数据。
